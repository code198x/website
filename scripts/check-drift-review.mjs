...
const output=process.env.DRIFT_REVIEW_OUTPUT || '/private/tmp/drift-browser';
const base=process.env.DRIFT_REVIEW_URL || 'http://127.0.0.1:4331/systems/sinclair-zx-spectrum/basic/drift/';
const samples=path.resolve(process.env.CODE_SAMPLES_PATH || '../code-samples');
const metadata=yaml.load(readFileSync(new URL('../src/content/units/sinclair-zx-spectrum/basic/drift.yaml',import.meta.url),'utf8'));
...
try {
 for(const device of ['desktop','mobile']) for(const theme of ['light','dark']) {
  ...
  for(const unit of units) {
   ...
   const mdx=readFileSync(new URL('../src/content/curriculum/sinclair-zx-spectrum/basic/drift/'+unit.slug+'.mdx',import.meta.url),'utf8');
   ...
   if([0,1,2,5,6,7,8].includes(unit.number)){await page.locator('details').evaluateAll(ns=>ns.forEach(n=>n.open=false));await page.evaluate(()=>scrollTo(0,0));await page.screenshot({path:path.join(output,`${unit.slug}-${device}-${theme}.png`)});}
