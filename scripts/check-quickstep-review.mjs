...
const output=process.env.QUICKSTEP_REVIEW_OUTPUT || '/private/tmp/quickstep-browser';
const base=process.env.QUICKSTEP_REVIEW_URL || 'http://127.0.0.1:4332/systems/sinclair-zx-spectrum/basic/quickstep/';
const samples=path.resolve(process.env.CODE_SAMPLES_PATH || '../code-samples');
const metadata=yaml.load(readFileSync(new URL('../src/content/units/sinclair-zx-spectrum/basic/quickstep.yaml',import.meta.url),'utf8'));
...
for (const unit of metadata.units) {
 const mdx=readFileSync(new URL('../src/content/curriculum/sinclair-zx-spectrum/basic/quickstep/'+unit.slug+'.mdx',import.meta.url),'utf8');
 const changed=mdx.match(/quickstep\/teaching\/([^/]+)\/changes\.bas/);
 if (!changed) continue;
 const edits=JSON.parse(readFileSync(path.join(samples,'sinclair-zx-spectrum/basic/quickstep/teaching',changed[1],'edits.json'),'utf8'));
