...
for(const width of [390,1440])for(const theme of ['light','dark']){
 ...
 for(const unit of [...roster,{slug:'',number:0}]){
  ...
  if(unit.number===7||unit.number===8){await page.locator('.question').first().scrollIntoViewIfNeeded();await page.screenshot({path:path.join(output,`lesson-${unit.number}-${width}-${theme}.png`)});}
