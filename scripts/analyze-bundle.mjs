...
function analyzeBundle() {
  ...
  if (analysis.js.length > 0) {
    ...
    analysis.js.forEach(file => {
      ...
      console.log(`  ${status} ${file.name.padEnd(40)} ${file.size.toString().padStart(8)} KB (${percentage}%)`);
    });
  ...
  console.log('\n🔬 Bundle Composition:');
  const jsPercentage = Math.round((analysis.js.reduce((sum, file) => sum + file.size, 0) / analysis.total.size) * 100);
  const cssPercentage = Math.round((analysis.css.reduce((sum, file) => sum + file.size, 0) / analysis.total.size) * 100);
