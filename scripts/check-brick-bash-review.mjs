...
const output=process.env.BRICK_BASH_REVIEW_OUTPUT || '/private/tmp/brick-bash-browser';
const base=process.env.BRICK_BASH_REVIEW_URL || 'http://127.0.0.1:4331/systems/sinclair-zx-spectrum/basic/brick-bash/';
const samples=path.resolve(process.env.CODE_SAMPLES_PATH || '../code-samples');
const metadata=yaml.load(readFileSync(new URL('../src/content/units/sinclair-zx-spectrum/basic/brick-bash.yaml',import.meta.url),'utf8'));
...
try {
 for(const device of ['desktop','mobile']) for(const theme of ['light','dark']) {
  ...
  for(const unit of units) {
   ...
   const mdx=readFileSync(new URL('../src/content/curriculum/sinclair-zx-spectrum/basic/brick-bash/'+unit.slug+'.mdx',import.meta.url),'utf8');
   ...
   if([0,1,6,8,10].includes(unit.number)){await page.locator('details').evaluateAll(ns=>ns.forEach(n=>n.open=false));await page.evaluate(()=>scrollTo(0,0));await page.screenshot({path:path.join(output,`${unit.slug}-${device}-${theme}.png`)});}
