...
const output=process.env.CRATES_REVIEW_OUTPUT || '/private/tmp/crates-browser';
const base=process.env.CRATES_REVIEW_URL || 'http://127.0.0.1:4327/systems/sinclair-zx-spectrum/basic/crates/';
const samples=path.resolve(process.env.CODE_SAMPLES_PATH || '../code-samples');
const roster=JSON.parse(readFileSync(new URL('../src/drafts/crates/roster.json',import.meta.url)));
...
for(const width of [390,1440]) for(const theme of ['light','dark']) {
 ...
 for(const unit of [{unit:0,slug:'index'},...roster].filter(unit => !process.env.CRATES_REVIEW_UNIT || unit.slug === process.env.CRATES_REVIEW_UNIT)) {
  ...
  const draft=readFileSync(new URL('../src/content/curriculum/sinclair-zx-spectrum/basic/crates/'+unit.slug+'.mdx',import.meta.url),'utf8');
  ...
  const links=await page.locator('main a[href], .unit-navigation a[href]').evaluateAll(as=>as.map(a=>a.getAttribute('href')).filter(h=>h.startsWith('/')||h.startsWith('.')));
  for(const href of new Set(links)){const target=new URL(href,page.url());const result=await context.request.get(target.href);if(result.status()!==200)throw Error('Broken link '+target.href); if(target.hash) { const html=await result.text(); const id=decodeURIComponent(target.hash.slice(1)); if(!html.includes('id="'+id+'"')) throw Error('Broken anchor '+target.href); }}
  if([0,1,2,6,8,10].includes(unit.unit)){await page.locator('details').evaluateAll(ds=>ds.forEach(d=>d.open=false));await page.evaluate(()=>scrollTo(0,0));await page.screenshot({path:path.join(output,`${unit.slug}-${width}-${theme}.png`)});}
