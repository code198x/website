...
test.describe('Cross-Browser Visual Consistency', () => {
  ...
  test('font rendering consistency', async ({ page, browserName }) => {
    ...
    const textElements = [
      'h1', 'h2', 'h3', 'p', '.nav-item__label', '.metric-label'
    ];
    ...
    for (const selector of textElements) {
      ...
      if (await element.isVisible()) {
        await expect(element).toHaveScreenshot(`font-${selector.replace(/[^a-z0-9]/gi, '-')}-${browserName}.png`, {
          animations: 'disabled',
        });
      }
    }
  ...
  test('accessibility focus indicators', async ({ page, browserName }) => {
    ...
    for (const selector of focusableElements) {
      ...
      if (await element.isVisible()) {
        ...
        await expect(element).toHaveScreenshot(`focus-${selector.replace(/[^a-z0-9]/gi, '-')}-${browserName}.png`, {
          animations: 'disabled',
        });
      }
    }
