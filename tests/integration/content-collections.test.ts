...
const mockTiers = [
  {
    ...
    phase: 0,
    tierNumber: 0,
    objectives: ["Convert between binary, hex, and decimal", "Understand bit manipulation"],
...
const mockLessons = [
  {
    ...
    tier: 0,
    lessonNumber: 0,
    objectives: ["Understand what binary is", "Count in binary from 0-15"],
...
describe("Content Collections Integration", () => {
    ...
    it("should avoid mastery terminology", () => {
      const allContent = [
        ...
        ...mockLessons.map((l) => ({
          ...
          wonderMoment: l.wonderMoment,
        })),
      ];

      allContent.forEach((content) => {
        const text =
          `${content.title} ${content.description} ${"wonderMoment" in content ? content.wonderMoment : ""}`.toLowerCase();
  ...
  describe("Tag System Validation", () => {
    it("should validate consistent tagging", () => {
      const allTaggedContent = [mockSystems[0], mockPhases[0], mockLessons[0]];
