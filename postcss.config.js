    ...
    ...(process.env.NODE_ENV === 'production' ? [
      ...
      purgeCSSPlugin({
        ...
        safelist: [
          ...
          /^performance-/, /^dashboard-/, /^metric-/, /^action-/,
          // Navigation classes
          /^nav-/, /^mobile-/,
          // Layout classes
          /^layout-/, /^content-/, /^footer-/, /^sidebar-/,
          // Component state classes
          /^is-/, /^has-/, /^--/, /-active$/, /-current$/,
          // Dark mode classes
          /^dark/, /dark$/,
          // Prism syntax highlighting classes
          /^token/, /^language-/, /^code/, /^pre/,
          // Common utility classes that might be used dynamically
          'hidden', 'visible', 'sr-only', 'skip-to-content',
        ...
        defaultExtractor: (content) => {
          ...
          const extractedClasses = classes.map(cls =>
            cls.match(/class(?:Name)?=["']([^"']+)["']/)[1]
          ).join(' ').split(/\s+/);
