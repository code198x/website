---
title: "Steering"
description: "The joystick arrives. Read the stick when a hop is available, move the sheep in 8-pixel steps, rewrite the sprite's POS/CTL — position is data, and now the player owns it."
pubDate: 2026-06-11
game: 1
unit: 3
...
When she hops, `cooldown` is set. Each later update with a non-zero count subtracts one and returns without reading the stick. Even the update that reduces it from one to zero returns; the following update can accept another hop. Hold *up* and she crosses the farm in a steady hop-hop-hop rhythm instead of gliding like a cursor. (Try `COOLDOWN equ 0` and feel the difference — she stops being an animal at all.)
...
What the assembler computed once at build time in Unit 2, the CPU now computes on each game update:
...
```asm
            move.l  #$1ff00,d1          ; Mask the combined beam line bits
...
Leaving line 0 before waiting for its return prevents a short loop from updating twice during the same visit to that line. Our work still needs to finish in time for the next visit. If it takes too long, we can miss a frame; the wait does not make that work faster. The two-phase wait fixes the repeated update in this example, rather than guaranteeing a frame rate for any workload.
...
3. **Count her speed.** At `STEP 8` / `COOLDOWN 6`, count the updates between accepted hops, including the update that reduces cooldown to zero. Predict the time for ten hops at a nominal 50 updates per second. Choose a stretch within the 0–240 position limits, and distinguish measuring from the first hop to the tenth from timing ten complete hop intervals. Compare with a recording or stopwatch; input timing and missed updates can affect the observation.
...
- **The two-phase beam wait** — avoid running twice during one visit to line 0; still keep the work within its timing budget.
