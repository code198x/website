...
## Verified from evidence to screen

This unit has an executable curriculum proof. It checks the Copper claims
against the 1985 *Amiga Hardware Reference Manual*, assembles this source with
Asm198x, masters the executable into a deterministic bootable ADF with
Build198x, then boots that disk on an A500 in Emu198x and checks the exact
768×576 output frame.

The public proof verifies every redistributable stage. The final boot check uses
a legally obtained Kickstart 1.3 ROM supplied locally and identified only by
its hash; the ROM is never included in the proof report.
