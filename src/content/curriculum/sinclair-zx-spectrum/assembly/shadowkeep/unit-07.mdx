...
![The pillared hall with a line of white chalk-cross marks across its floor, the red thief standing among them — the trail he laid before walking to the next room and back, still exactly where he left it.](/images/sinclair-zx-spectrum/assembly/shadowkeep/unit-07/screenshot.png)
...
<CodeFromFile src="sinclair-zx-spectrum/assembly/shadowkeep/unit-07/the-hero-remembers.asm" />

## Assemble and run

```
pasmonext --sna the-hero-remembers.asm the-hero-remembers.sna
```

Load it in Emu198x. Hold **SPACE** and walk **Q A O P** to lay a chalk trail. Walk it out through the east door into the next room, turn around, and walk back. Your marks are waiting. Chalk a different shape in the second room and ferry between the two — each keeps its own.

## Try this: the honest cost

Two rooms cost two 768-byte buffers — 1,536 bytes of RAM just for state. Work out what a hundred-room keep would cost that way (it's most of the machine). This is the *honest* version: a full mutable copy per room, simple and clear. A bigger keep needs the state packed far tighter — a bitmap of "marked or not", or a short list of changed cells. Worth sketching how you'd shrink it; it's a real problem real games solved.
...
- **Persistence is the absence of overwriting.** Once each room's state lives in its own buffer and you draw *from* it, anything you write there simply stays. You don't add memory; you stop discarding it.
...
- **The simple version has a cost.** A full copy per room is clear but heavy; knowing where it won't scale is the first step to the version that does.
