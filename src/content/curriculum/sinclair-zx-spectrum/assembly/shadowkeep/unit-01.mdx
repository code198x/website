...
description: "Shadowkeep begins where The Long Night ended — the same engine, a new face. Design a hooded thief as eight bytes and stand him, alone, in the cold blue dark of the keep."
pubDate: 2026-06-02
game: 2
unit: 1
...
You have already finished a game — and then made it better. Gloaming, played from a title and back again; and across *The Long Night*, the same game deepened into a five-watch siege with light, mood and a voice. Shadowkeep is the next one, and it is bigger in a new direction: not a deeper screen, but a *keep* — a lit, multi-room place you explore by night. But it does not start from nothing, and it does not start from a blank file.

It starts from the engine you finished — the cell sprite, the save-and-restore, the collision, the game loop, the state machine, all first built in Gloaming and lived-in across the long night. You wrote every line of it, and Shadowkeep carries it over wholesale. So the first thing a new game needs isn't new code. It's a **new face**.
...
- **Shadowkeep stands on the engine you built.** The border, the `LDIR` wash, `scr_addr_cr` / `attr_addr_cr`, the `inc h` draw loop — all carried over from Gloaming and lived-in across The Long Night. A new game reuses a finished engine; it doesn't rebuild it.
