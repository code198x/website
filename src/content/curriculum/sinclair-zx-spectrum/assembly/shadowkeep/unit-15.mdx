...
![The Great Hall, lit and waiting, two coins of gold on its floor — the screen you've built, now opening to a short theme played as the keep comes into view.](/images/sinclair-zx-spectrum/assembly/shadowkeep/unit-15/screenshot.png)

The screen is the Hall as before — but now, as the keep opens, a short solemn phrase plays: a melody in D minor, rising to a held note and falling home, then a brief answer. Six or seven seconds, one voice, then the game begins. It's the sound of the keep introducing itself. (Load it and listen — the still is silent, the keep is not.)
...
<CodeFromFile src="sinclair-zx-spectrum/assembly/shadowkeep/unit-15/a-theme-in-one-voice.asm" />

## Assemble and run

```
pasmonext --sna a-theme-in-one-voice.asm a-theme-in-one-voice.sna
```

Load it with the sound up. The theme plays as the Hall appears, then the game begins as before — explore, collect, win. The keep now has a voice that's *its own*, not a generic blip.
...
True two-part harmony is beyond one bit — but you can *fake* it by alternating two pitches very fast within a chunk, so the ear hears a chord-like shimmer. Sketch it on two notes. It's how advanced Spectrum music squeezed apparent polyphony from a single speaker — and a glimpse of where beeper craft can go.

## If it doesn't work

- **One long tone instead of a tune.** The player isn't advancing `HL`, or every note has the same pitch. Each pair must `inc hl` twice; check the pitches in the data actually differ.
