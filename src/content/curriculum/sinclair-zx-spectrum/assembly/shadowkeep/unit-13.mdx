...
You know this driver cold, so take it as a reminder, not a lesson. The speaker is a single bit — bit 4 of port `$FE`; toggle it high, low, high, fast, and the cone makes a square wave. `beep` takes two numbers — **B**, how many cycles to play (the duration), and **C**, how long to wait between toggles (the pitch; a larger `C` waits longer, so the wave is slower and the note lower). It is the same primitive that rang the dusk bells; the only thing that changes, unit to unit, is what you ask of it:
...
<CodeDiff
  ...
  lang="z80"
  title="Step 1: a one-bit beeper driver, a footfall and a door creak"
  context={3}
/>
...
<AudioClip
  src="/images/sinclair-zx-spectrum/assembly/shadowkeep/unit-13/step-01-footsteps.wav"
...
<AudioClip
  src="/images/sinclair-zx-spectrum/assembly/shadowkeep/unit-13/step-01-door.wav"
