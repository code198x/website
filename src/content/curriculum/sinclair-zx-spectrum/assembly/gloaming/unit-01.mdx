...
Each step is a complete program, and the course's sources are written in **pasmo syntax** — so two assemblers build them, to the same program bytes. **Asm198x** is the 198x family's own assembler, one binary that covers this and every other platform in the course:

```
asm198x --dialect pasmo --cpu z80 --tapbas steps/step-03.asm -o steps/step-03.tap
```

Or use [**Pasmo**](https://pasmo.speccy.org/) itself:

```
pasmo --tapbas steps/step-03.asm steps/step-03.tap
```

Either way, open the tape in your emulator, type `LOAD ""` and start the tape. The program writes the whole square in well under a frame, then idles — so the moment it runs, the walled square is there.
