...
To texture *any* cell, we need its bitmap address from its column and row at run time. This is `scr_addr_cr`, and it's pure bit-shuffling — the row number splits into two pieces that land in different halves of the address:

```
row  = %000ttrrr        col = %000ccccc

H = %010t t000          the third, under the $4000 screen base
L = %rrrc cccc          char row within the third, then the column
```
