...
```
asm198x --dialect pasmo --cpu z80 --tapbas bigger-than-a-byte.asm -o primer.tap
```
...
- **The assembler errors on the add.** Sixteen-bit adds go through `HL`: it's `add hl, de`, not `add hl, 32`. Load the amount into `DE` first with `ld de, 32`.
