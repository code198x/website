...
There's a catch you already half-know: `A` holds eight bits, 0–255. Add past 255 and it can't hold the answer — it **wraps round and sets the carry flag**: 255 + 1 gives 0, 255 + 2 gives 1. That's the carry we borrowed in *Test, Then Jump*; here's where it comes from. (More in the Try this.)

## What you'll see by the end

<Figure src="/images/sinclair-zx-spectrum/assembly/meet-the-machine/unit-12/cyan-border.png" alt="The screen surrounded by a cyan border." caption="A cyan border — colour 5, never loaded but computed: start at 2, inc twice to 4, add 1 to 5, then show it." />
...
```
asm198x --dialect pasmo --cpu z80 --tapbas arithmetic.asm -o primer.tap
```
...
- **The assembler errors on `add 1`.** The 8-bit add names its target: it's `add a, 1`, not `add 1`. (`sub`, oddly, is just `sub 1` — `A` is assumed.)
...
`inc`/`add` add and `dec`/`sub` take away. All four wrap a result past 255 or below 0, but only `add` and `sub` set the carry flag, the machine's overflow signal — `inc` and `dec` leave it alone.
