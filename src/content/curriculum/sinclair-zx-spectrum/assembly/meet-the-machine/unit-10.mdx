...
<Figure src="/images/sinclair-zx-spectrum/assembly/meet-the-machine/unit-10/red-bar.png" alt="A solid red bar running the full width of the screen, near the top." caption="The whole top row — 32 colour cells — painted by a two-line body that ran 32 times. You wrote the body once; DJNZ did the counting." />
...
```
asm198x --dialect pasmo --cpu z80 --tapbas counting-toward-zero.asm -o primer.tap
```
...
<CodeDiff
  before="sinclair-zx-spectrum/assembly/meet-the-machine/unit-10/counting-toward-zero.asm"
  after="sinclair-zx-spectrum/assembly/meet-the-machine/unit-10/fill-pixel-row.asm"
  lang="z80"
  title="The same loop pointed at $4000, writing the dotted byte"
  context={2}
/>
...
- **The assembler can't find `.fill`.** The label and the `djnz .fill` must match exactly, case and all — labels are the assembler's rules, from Beat 7.
