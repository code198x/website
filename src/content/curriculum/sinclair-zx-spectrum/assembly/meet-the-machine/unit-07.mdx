...
<Figure src="/images/sinclair-zx-spectrum/assembly/meet-the-machine/unit-07/green-border.png" alt="The Spectrum's default screen surrounded by a solid green border." caption="A green border — because the program decided on green: A was 5, the compare matched, and it jumped to the green road." />
...
- **Each name can be used once.** The leading dot is part of the name, and our habit for a label that only matters to the routine above it. The assembler doesn't enforce that: every label must be unique across the whole program, so two routines can't both have a `.loop`.

Here's the important part, though. Everything you've learned about the *machine* — registers, the screen, the flags — is **universal**: it's true of any Z80, assembled by any tool. But how you *write* labels is a rule of the **assembler** — the program that turns your text into bytes — not of the chip. We write in **Pasmo's** notation; a different assembler might treat label case differently, or have true local labels that each routine can reuse. This is the first time the *tool* has a say, not just the machine. From here on, when a rule belongs to the assembler and not the Z80, we'll say so.

## Assemble and run

```
asm198x --dialect pasmo --cpu z80 --tapbas test-and-jump.asm -o primer.tap
```
...
<CodeDiff
  before="sinclair-zx-spectrum/assembly/meet-the-machine/unit-07/test-and-jump.asm"
  after="sinclair-zx-spectrum/assembly/meet-the-machine/unit-07/three-way.asm"
  lang="z80"
  title="A second jump on the carry flag splits less / equal / greater"
  context={2}
/>
...
- **The assembler says a symbol is undefined.** A label's spelling doesn't match between where you defined it and where you jump to it — most often a capital letter. Labels are case-sensitive: `.equal` ≠ `.Equal`.
