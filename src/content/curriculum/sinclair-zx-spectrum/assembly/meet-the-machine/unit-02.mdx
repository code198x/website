...
```
asm198x --dialect pasmo --cpu z80 --tapbas registers.asm -o primer.tap
```

Load `primer.tap` in your emulator with a register view open, and watch `A` and `B` move through the table above before the border settles on cyan.
...
- **The assembler says "Expected 'A' but…" on the `out` line.** You tried to `out` a register other than `A`. Only `A` can do `out ($FE), a` — route the value back into `A` first.
