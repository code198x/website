...
```
asm198x --dialect pasmo --cpu z80 --tapbas everything-is-a-number.asm -o primer.tap
```
...
- **The assembler errors on the `'A'` line.** Use straight single quotes around exactly one character — `'A'`, not `‘A’` (curly quotes) and not `"A"` (double quotes make a string, not a byte).
