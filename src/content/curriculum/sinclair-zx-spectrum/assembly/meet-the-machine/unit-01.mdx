...
<Emulator
  ...
  src="/code-samples/sinclair-zx-spectrum/assembly/meet-the-machine/unit-01/border.tap"
...
From this unit's directory, turn the source into a tape the Spectrum can load. The course's sources are written in **pasmo syntax**, and **Asm198x** — the 198x family's own assembler, one binary that covers this and every other platform in the course — builds them:

```
asm198x --dialect pasmonext --tapbas border.asm -o primer.tap
```

Or use **pasmonext** itself, the enhanced Pasmo the dialect is named for:

```
pasmonext --tapbas border.asm primer.tap
```

That's the build step — `border.asm` (the text you wrote) goes in, `primer.tap` (a program the Spectrum can load) comes out. Open it in your emulator, type `LOAD ""` and start the tape, and the border turns red.

**A tape, rather than a snapshot.** A snapshot is a photograph of a machine already running, and an assembler making one has to invent everything the program didn't set. A tape is the program, loaded the way the Spectrum loads anything: the machine switches on, its ROM sets itself up, and *then* your code runs. That costs a few seconds and buys you a real machine underneath — which matters the moment a program wants the ROM to do something for it, like printing a character.
...
If you haven't got the tools installed yet, you can run the same loop in this page. It's the same assembler — Asm198x, compiled to run in a browser — producing the same tape the command above produces. Change the source, press the button, and the Spectrum loads what you just built.
...
- **The assembler reports an error and writes no `.tap`.** Read the line number it gives — usually a mistyped instruction or a missing comma. Fix that one line and assemble again; the rest of the file is fine.
- **The border doesn't change — it's still white (or whatever it was).** The tape didn't finish loading, or it's an old build. Re-run the assembler, then load the freshly-written `primer.tap`.
