...
This is the Primer — *Meet the Machine*. Its whole job is to make the [Spectrum](/vault/systems/sinclair-zx-spectrum) stop feeling like magic, one small idea at a time. No game yet, no sprites, no clever code. Just you and the machine, getting introduced.
...
`org 32768` tells the assembler where the program goes in memory; everything after it is laid out from that address. `ld a, 2` puts the number 2 into register `A`. `out ($FE), a` sends it through the door to the border. The `halt` / `jr` pair at the end just holds the picture still — without it the machine would carry on past our two instructions into whatever bytes happen to come next, and do something we never asked for. (That "the machine just keeps going" is a big idea; we'll meet it properly at the end of the Primer.)
...
```
asm198x --dialect pasmo --cpu z80 --tapbas border.asm -o primer.tap
```

Or use [**Pasmo**](https://pasmo.speccy.org/) itself:

```
pasmo --tapbas border.asm primer.tap
```
