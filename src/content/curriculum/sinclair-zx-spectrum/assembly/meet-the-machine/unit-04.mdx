...
In **memory**. Picture one long street of numbered boxes — 65,536 of them on the [Spectrum](/vault/systems/sinclair-zx-spectrum). Each box has an **address** (just a number, like a house number) and holds exactly one byte. The whole street is yours: you can write a byte into any box and read it back out whenever you like.
...
```
asm198x --dialect pasmo --cpu z80 --tapbas numbered-boxes.asm -o primer.tap
```
...
- **The assembler errors on the store/read line.** A bracket is missing or unbalanced. Store is `ld ($9000), a`; read is `ld a, ($9000)`.
