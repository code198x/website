...
the block for the current level, so the *same* loader reads level 1, 2 or 3 depending on `lv`. A fixed line after
`RESTORE` selects one DATA section; here the line is *computed* from a variable, which turns one
...
That closes **Volume 2 — Patterns of State**. Across the volume you have held state as a hidden value,
