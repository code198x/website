...
To enter line 70, type its number, press U for `IF`, then type `g`. Switch to
**Original Keyboard**, hold Alt/Option and press W for `<>`, then return to
**Host Keyboard**. Type `secret`. Enter `THEN` with Alt/Option+G in Original
mode, switch back, then press G for `GO TO` and type `30`. Press Enter to
store the line. The keyboard toggle is Cmd+Shift+K on macOS or Ctrl+Shift+K on
Windows and Linux.
...
switch to Original Keyboard, press and release **Shift+Alt/Option** together,
then press **R**. The shift chord selects the Spectrum’s extended keyword
mode; R then enters `INT`. Return to Host Keyboard to type `g`. Continue with
`THEN GO TO 100` using the keyword keys already described.

For line 90, type the number, switch to Original Keyboard and press
**Alt/Option+A** for `STOP`. Release the keys and return to Host Keyboard,
then press Enter. `STOP` ends execution here. It prevents a successful round
from continuing into the rejection message below.
