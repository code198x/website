...
**Tab and press W** for `<>`, then release both keys. Type `secret`, use
**Tab+G** for `THEN`, release the keys, then press G for `GO TO` and type
...
press and release **Shift+Tab** together, then press **R**. The shift chord
...
For line 90, type the number, hold **Tab and press A** for `STOP`, release
