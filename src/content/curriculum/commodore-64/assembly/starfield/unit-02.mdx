...
If the ship moves the wrong way, that byte tells you whether the problem is your reading (wrong bit) or your moving (wrong `INC`/`DEC`). If pushing up gives you `$fd` instead of `$fe`, you're testing down's bit, not up's. Watching the value the program actually sees beats re-reading your own code hoping to spot the slip.
