...
Three enemies need three X positions, three Ys, three flash timers. Rather than naming them all, we lay each set out as a **parallel array** in zero page — consecutive bytes, one per enemy — so the *same* index reaches the matching slot in every array:
