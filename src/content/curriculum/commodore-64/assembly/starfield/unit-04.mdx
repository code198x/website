...
Unit 3 left a ship that fires a bullet. It works, but it's silent.

![The yellow bullet rising from the ship towards the top of the screen — Unit 3's shot, with no sound.](/images/commodore-64/assembly/starfield/unit-04/shot.png)
...
<AudioClip
  src="/images/commodore-64/assembly/starfield/unit-04/step-01-silent.wav"
...
<AudioClip
  src="/images/commodore-64/assembly/starfield/unit-04/step-02-blip.wav"
...
<CodeDiff
  before="commodore-64/assembly/starfield/unit-04/steps/step-02.asm"
  after="commodore-64/assembly/starfield/unit-04/steps/step-03.asm"
  lang="asm"
...
<details>
<summary>The complete program</summary>
<CodeFromFile src="commodore-64/assembly/starfield/unit-04/steps/step-03.asm" />
</details>

Now it pews:

<AudioClip
  src="/images/commodore-64/assembly/starfield/unit-04/laser.wav"
  title="Step 3: the pew — the pitch slides from high to low as the shot fires"
  source="SID 6581 · recorded from a real shot"
/>

That's the whole difference between a sound and a *laser*: the same gated sawtooth, its pitch dropped from a bright high tone to a low one in under a fifth of a second, fading to silence as it falls. Unit 3 let you *see* the bullet leave; now you *hear* it go. And the tuning — how high it starts, how fast it falls — is exactly the kind of single number that turns "it works" into "it feels good." Start the sweep higher for a sharper zap; fall slower for a longer, lazier pew.

## When it's wrong, see why

Silent SID code is the most common frustration on this machine, and guessing rarely finds it. Open the SID register view in Emu198x and check the chain a sound needs, in order:
...
- **Sweep** — no "pew", just a blip? Watch your zero-page copy of the pitch (`laser_freq`): does it get set high on the shot and count down? If it never changes, the sweep block isn't running — and remember you can't read `$d401` back to check, because it's write-only.

Step 1 was a controlled version of exactly this — every register set *except* the gate. Reading the chip's state (and your own variables) and finding the one link that's missing beats re-reading your code hoping to spot it.
