...
![The C64 screen with a red border, fire held down.](/images/commodore-64/assembly/meet-the-machine/unit-08/screenshot.png)
...
This reads exactly **one direction, one bit**. The whole stick is five bits, and reading several at once — up, down, left, right, fire, all in a pass — is a job of its own. We're leaving it here: that belongs to the **first game**, where moving a ship actually needs it. For now, the machine can hear one push, and that's the whole idea.
