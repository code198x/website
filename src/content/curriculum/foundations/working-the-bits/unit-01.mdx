...
game: 1
unit: 1
tags: ["foundations", "working-the-bits", "bit-logic"]
---
...
- **You reached for `AND` and got a yes or no, not a mask.** That is the question-joining `AND`
  from *Decisions*. Masking wants `BITAND`.
...
These tools change *which* switches are on. In **Unit 2** we do something different, and slide
