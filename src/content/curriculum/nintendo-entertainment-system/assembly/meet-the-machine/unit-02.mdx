...
In BASIC, `LET A = 5` gave you a variable, and you could have as many as you liked. The [2A03](/vault/systems/nintendo-entertainment-system) — the NES's 6502 — is not so generous. It has **three** registers — small stores *inside the chip itself* — and that's all. Their names are fixed: **`A`** (the *accumulator*, where most work happens) and **`X`** and **`Y`** (two helpers we'll lean on later). You don't declare new ones. You learn to *orchestrate the three you have*.
