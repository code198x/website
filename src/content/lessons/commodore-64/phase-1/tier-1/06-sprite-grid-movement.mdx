...
**"Good! Every move is exactly one grid cell!"**

By the end of this lesson, your sprite will move in discrete grid steps - no more continuous pixel movement. Press a direction and your sprite will snap from one grid cell to the next, demonstrating basic grid-based movement.

![Screenshot of completed lesson](/images/c64-foundation-lesson-06.png)

## Why Grid Movement Matters

**"This teaches basic discrete movement concepts!"**

Grid-based movement is useful for:
...
Grid movement creates precision and predictability - the opposite of free-form pixel movement.
...
**Key Point**: Five ASL instructions = multiply by 32! The reverse of our division trick.
...
**Key Point**: We only move when there's input, and we move exactly one grid cell.
...
**Key Point**: Your sprite moves exactly one grid cell per input, with boundary checking.
...
**Key Point**: Basic grid movement with automatic boundary checking.
...
**Key Point**: Your sprite now moves in discrete grid steps.
...
1. **Create the program**: All code is in `grid-protocol-06.asm`
...
Your position conversion works correctly:
- **Grid (3,2) → Pixel (96,114)**: 3×32=96, 2×32+50=114
- **Grid (7,5) → Pixel (224,210)**: 7×32=224, 5×32+50=210
- **Centered**: Sprite sits in the middle of each cell
...
🎉 **Well Done!** You've implemented basic grid-based movement!

### What You Built
- ✨ **Discrete movement** - One-cell-per-move precision
- ✨ **Grid-to-pixel conversion** - Reverse coordinate transformation
- ✨ **Boundary checking** - Basic grid limit enforcement
...
You've implemented a basic movement system similar to what's used in classic games. This discrete, grid-based movement is a foundation for strategic gameplay, puzzle mechanics, and tactical positioning.

### Historical Context
This movement system is similar to what's used in:
...
### Grid Boundaries
- **X range**: 0-11 (12 columns)
- **Y range**: 0-7 (8 rows)
- **Pixel coverage**: Full screen with lookup tables
- **Cell spacing**: Variable spacing from lookup tables

### Movement Variables
- **grid_x/y**: Logical position (0-11, 0-7)
- **sprite_x/y**: Pixel position (from lookup tables)
- **Direction checking**: One move per input

This grid movement system provides a foundation for games that need discrete positioning.