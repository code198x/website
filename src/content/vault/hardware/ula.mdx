...
machine disagreeing about whether a refresh cycle counts as an access. It follows
from the mechanism that the effect depends on where the `I` register happens to
point, since `I` supplies the high byte of the refresh address — though the book
as scanned here describes the cause rather than prescribing a remedy.
