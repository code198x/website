...
Nintendo's own documents never call it "MMC1". They call the part simply **the MMC**, expanded on the first page of its programming specification as *Multi Memory Controller*. The number only appears once a second controller existed to distinguish it from.
...
On MMC1A the same bit did something else entirely, driving an external pin for RAM bank selection. The two revisions are therefore not quite interchangeable, which is why emulator authors have to decide which part a given game expects rather than implementing "MMC1" as one thing.
