...
The Blitter operates on one word (16 pixels) per cycle in 4-channel cookie-cut mode (A read + B read + C read + D write = 4 cycles per word). For a 32×32 pixel BOB on a 5-bitplane playfield:

- 32 pixels = 2 words horizontally
- 32 lines tall × 5 bitplanes = 160 line-passes
- 2 words × 160 passes × 4 cycles = **1,280 Blitter cycles per BOB**

At 7.09 MHz Amiga PAL, that's about **180 microseconds** per BOB — 0.9% of a 20ms frame. So 10-15 medium BOBs comfortably fit, and the practical limit is closer to 30-40 if you keep them small or single-plane.

Typical budget: 10-20 medium BOBs at 50 fps with playfield + game logic still running.
...
Famous example: *Lionheart* (1993) uses hardware sprites for the parallax foreground decorations and BOBs for the player + enemies — opposite of the typical layout, taking advantage of sprites' freedom to overlap the playfield without bookkeeping.
