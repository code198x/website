...
subtitle: "BASIC memory management"
summary: "How variables work internally and memory management for BASIC programs"
category: "techniques"
platforms: ['commodore-64']
tags: ['reference', 'variables', 'memory', 'basic']
sources:
  storage_sizes:
    - title: "Commodore 64 Programmer's Reference Guide, Commodore Business Machines"
      kind: book
      page: "5, 8"
      note: "\"Floating-point numbers are stored (using five bytes of memory) and are manipulated in calculations with ten places of accuracy.\" — \"The amount of memory required to store an array can be determined as follows: 5 bytes for the array name + 2 bytes for each dimension of the array + 2 bytes per element for integers OR + 5 bytes per element for floating-point OR + 3 bytes per element for strings AND + 1 byte per character in each string element\""
  fre:
    - title: "Commodore 64 Programmer's Reference Guide, Commodore Business Machines"
      kind: book
      page: "48-49"
      note: "FRE: \"This function tells you how much RAM is available for your program and its variables. If a program tries to use more space than is available, the ?OUT OF MEMORY error results. The number in parentheses can have any value, and it is not used in the calculation. NOTE: If the result of FRE is negative, add 65536 to the FRE number to get the number of bytes available in memory.\" — \"The following always tells you the current available RAM: PRINT FRE(0)-(FRE(0)<0) * 65536\""
  lowering_the_top:
    - title: "Commodore 64 Programmer's Reference Guide, Commodore Business Machines"
      kind: book
      page: "108-109"
      note: "\"To protect your character set from BASIC, you should reduce the amount of memory BASIC thinks it has. The amount of memory in your computer stays the same… it's just that you've told BASIC not to use some of it. Type: PRINT FRE(0)-(SGN(FRE(0))<0)*65535 … Now type the following: POKE 52,48: POKE56,48: CLR\" — \"See the change? BASIC now thinks it has less memory to work with. The memory you just claimed from BASIC is where you are going to put your character set, safe from actions of BASIC.\""
  memory_maps:
    - title: "Commodore 64 Programmer's Reference Guide, Commodore Business Machines"
      kind: book
      page: "262-263"
      note: "LORAM=1, HIRAM=1: \"This is the default BASIC memory map which provides BASIC 2.0 and 38K contiguous bytes of user RAM.\" — LORAM=1, HIRAM=0: \"This map provides 60K bytes of RAM and I/O devices. The user must write his own I/O driver routines.\" — LORAM=0, HIRAM=1: \"This map is intended for use with Softload languages (including CP/M), providing 52K contiguous Bytes of user RAM, I/O devices, And I/O driver routines.\""
---
...
Surprisingly, integer variables still use 7 bytes. The `%` just tells BASIC to truncate values to integers when assigned. There's no memory advantage — the saving only appears in arrays, where the manual's formula gives "2 bytes per element for integers" against 5 for floating-point.
...
| Bytes | Field |
|-------|-------|
| 0-1 | Variable name (high bit of byte 1 set marks it as a string) |
| 2 | String length (0-255) |
| 3-4 | Pointer to string data in the heap |
| 5-6 | Unused / padding |
...
**Storage:** 5 bytes for the name + 2 bytes per dimension + (5 bytes × elements) — the Programmer's Reference Guide's own formula

```basic
dim a(10)        rem 5 + 2 + (5 × 11) = 62 bytes
dim b(100)       rem 5 + 2 + (5 × 101) = 512 bytes
dim grid(20,20)  rem 5 + 4 + (5 × 21 × 21) = 2214 bytes
```
...
**Storage:** 5 bytes for the name + 2 bytes per dimension + (2 bytes × elements)

```basic
dim a%(10)       rem 5 + 2 + (2 × 11) = 29 bytes
dim b%(100)      rem 5 + 2 + (2 × 101) = 209 bytes
```
...
This is the manual's own recipe. Its programmable-characters chapter has the reader print `FRE(0)`, type `POKE 52,48: POKE56,48: CLR`, and print it again: "See the change? BASIC now thinks it has less memory to work with. The memory you just claimed from BASIC is where you are going to put your character set, safe from actions of BASIC." (Location 52 is the high byte of the string-storage pointer; `CLR` resets it from the top-of-memory pointer anyway, which is why poking 55 and 56 alone also works.)

**Where the top already is:** the default map gives BASIC "38K contiguous bytes of user RAM", from `$0800` to `$9FFF`, so the top-of-BASIC pointer starts at `$A000`. The 4 KB at `$C000-$CFFF` is already outside BASIC's reach — you do not need to move anything to use it — and there is no sense in setting the pointer higher than `$A000`, since the BASIC ROM sits there. Lowering it is only about carving a hole *below* `$A000`.

| Top of BASIC | POKE values | Claimed from BASIC | Notes |
|--------------|-------------|--------------------|-------|
| `$8000` (32768) | `POKE 56,128:POKE 55,0:CLR` | $8000-$9FFF = **8 KB** | Plus the free 4 KB at `$C000` |
| `$3000` (12288) | `POKE 56,48:POKE 55,0:CLR` | $3000-$9FFF = **28 KB** | The manual's example; common for ML game engines that load high |

**Warning:** Must do this **before** defining arrays or variables. `CLR` wipes everything.

**Banking the ROMs out is not a BASIC option.** The Programmer's Reference Guide's map for LORAM=1, HIRAM=0 "provides 60K bytes of RAM and I/O devices" — and adds that "the user must write his own I/O driver routines", because in that map the BASIC ROM is gone along with the KERNAL. A running BASIC program that clears HIRAM stops running. Those maps belong to machine-code programs that have taken over the machine.
...
- [BASIC V2 Reference](/vault/techniques/basic-v2) — complete variable syntax
- [Random Numbers](/vault/techniques/random-numbers) — RND seed storage location
- [Screen Memory](/vault/techniques/screen-memory) — using POKE for direct screen writes
