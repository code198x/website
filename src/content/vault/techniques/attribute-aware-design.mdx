...
For games that encode game rules in attribute bytes (the *Atic Atac* lineage, including [Shadowkeep](/curriculum/sinclair-zx-spectrum/assembly/game-01-shadowkeep)), the attribute byte is doing two things at once: visually presenting the cell to the player AND encoding what the cell *means* (wall, floor, door, treasure). A single `LD A, (HL)` reads both — the rendering and the rule are the same byte.
