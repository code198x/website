...
Any positive argument behaves like `RND(1)`; the value itself is ignored.
...
**Quality:** Adequate for game development — enemy AI, loot tables, dice rolls. Not suitable for cryptography, statistical simulation, or anything where adversarial prediction matters. (For cryptographic randomness on retro hardware, the conventional trick is to read the SID's noise channel via `$D41B`.)
