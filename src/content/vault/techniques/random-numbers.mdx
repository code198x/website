...
**Why?** The generator's seed lives at zero-page addresses `$8B-$8F` (139-143 decimal) as a 5-byte BASIC float. The KERNAL initialises this float to a fixed power-on value, so every reset starts the sequence at the same point.
