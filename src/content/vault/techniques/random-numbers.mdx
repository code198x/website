...
Fisher–Yates gives equally likely permutations when each choice is uniform over its remaining range and independent of earlier choices. A particular generator and range mapping need their own checks; the loop alone does not establish those properties.
...
```basic
200 rem choose 10 cards in the last 10 slots
...
This loop selects the ten cards in positions 42–51. Read those slots for the draw;
the first ten positions have not been selected by this procedure. Stopping earlier
saves swaps when we need only part of the deck.
...
**Limitation:** This fragment assumes at least one unused position remains. Once
all ten are used, the retry loop cannot finish. Even before that, its number of
draws varies. Track how many remain and return “no space” when empty, or use the
shuffle-and-iterate approach below when a bounded amount of selection work matters.
...
A fixed seed repeats the generator sequence only with the same generator and call order. Repeating game behaviour also needs matching initial state, input timing and any external values mixed into the generator.

Choose deliberately whether the released game uses fixed challenges or varied seeds. Keep a repeatable diagnostic route either way.
...
**Period:** Do not infer a sufficient period from a few varied-looking results. Check the actual generator and how often the game calls it; reseeding also changes reproducibility.
