...
Colour RAM at `$D800` is a separate chip that `$D018` never touches, so it "can't be doublebuffered" and has to be updated while the rows concerned are off-screen — see [Hardware Scrolling](/vault/techniques/hardware-scroll) for how games worked round that.

The flip also simplifies a scrolling copy. With one screen, Cadaver writes, "one must be careful in what direction the shifting loops should go, to not wipe out the entire screen data"; with two, the data "is always being copied from the currently visible screen to the other, hidden screen", and by using X for the source index and Y for the target "all 8 directions can be achieved by simply adjusting the initial index register values". The price is two copy loops, one for each direction of copy. Malcolm Bamber's 4-way scroller keeps its pair at 1024 and 3072 with a zero-page flag recording which one is showing.
