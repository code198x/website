...
sources:
  the_minimal_c64_player:
    - title: "Codebase64, 'Simple IRQ Music Player'"
      kind: technical-doc
      page: "sid:simple_irq_music_player"
      note: "The whole pattern in two calls: 'jsr $1000 ;Initialize Richard's music' at setup, then inside the handler 'lda #$01 / sta $d019 ; ACK any raster IRQs / jsr $1003 ;Play the music / jmp $ea31'. Init at the load address, play three bytes later, acknowledge before doing anything else"
  the_file_format_gotcha:
    - title: "Frantic, notes on 'Simple IRQ Music Player', Codebase64"
      kind: technical-doc
      page: "sid:simple_irq_music_player"
      note: "'.sid files have a header of length $7c bytes. You must remove this header from the sid files before you can place them on $1000 in c64 memory (for sids that should indeed be placed on $1000, which is something like a standard, but not every tune follow this).' In ACME the skip is written into the include directive, and the +2 'also removes the load address (two bytes) from the original c64 file inside the .sid file'"
  the_music_call_is_not_free:
    - title: "mixer, 'ADSR Discussion Notes', Codebase64"
      kind: technical-doc
      page: "base:classic_hard-restart_and_about_adsr_in_generally"
      note: "Why the once-per-frame call shapes the driver: 'Sound-routines are often called once per frame. One frame is is 1/50 second interval. The HR is started 2 full frames before next note' - so the interrupt rate sets the granularity at which a driver can schedule its hard restarts"
...
|--------|----------|-------|
| Rob Hubbard's custom drivers | C64 | Hand-rolled per game; *Monty on the Run*, *Commando* etc. each have their own engine. Disassembled and annotated by Anthony McSweeny in *C= Hacking* 5 |
