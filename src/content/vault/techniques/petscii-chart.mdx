...
sources:
  two_tables:
    - title: "Commodore 64 Programmer's Reference Guide, Commodore Business Machines"
      kind: book
      page: "376"
      note: "Appendix B, Screen Display Codes: \"The following chart lists all of the characters built into the Commodore 64 character sets. It shows which numbers should be POKEd into screen memory (locations 1024 – 2023) to get a desired character. Also shown is which character corresponds to a number PEEKed from the screen.\" — \"Two character sets are available, but only one set at a time… From BASIC, POKE 53272,21 will switch to upper case mode and POKE 53272,23 switches to lower case.\" — \"The reverse character code may be obtained by adding 128 to the values shown.\""
    - title: "Commodore 64 Programmer's Reference Guide, Commodore Business Machines"
      kind: book
      page: "379"
      note: "Appendix C, ASCII and CHR$ Codes: \"This appendix shows you what characters will appear if you PRINT CHR$(X), for all possible values of X. It will also show the values obtained by typing PRINT ASC(\"x\"), where x is any character you can type. This is useful in evaluating the character received in a GET statement, converting upper/lower case, and printing character based commands (like switch to upper/lower case) that could not be enclosed in quotes.\""
  the_name:
    - title: "Commodore 64 Programmer's Reference Guide, Commodore Business Machines"
      kind: book
      page: "227, 262"
      note: "The manual's own name for the PRINT/CHR$ code is \"CBM ASCII\": \"there is a machine language subroutine that will PRINT a character to the screen. The CBM ASCII code of the character should be in the accumulator before calling the subroutine. The address of this subroutine is $FFD2.\" — the autostart cartridge signature ends with \"the digits '80' in PET ASCII\""
---
