...
The Programmer's Reference Guide explains each bit in the same terms: clear LORAM and "the BASIC ROM will disappear from the memory map and be replaced by 8K bytes of RAM from $A000 to $BFFF"; CHAREN "is used only to bank the 4K byte character generator ROM in or out", which "the microprocessor only needs to access… when downloading the character set from ROM to RAM". It also states the rule that makes the C64's banking cheap to live with: "a WRITE (a POKE) to a ROM location will store data in the RAM 'under' the ROM", so "a hi-resolution screen [can] be kept underneath a ROM, and be changed without having to bank the screen back into the processor address space" — only a read returns the ROM. Programs can fill the 64K without ever switching the ROMs out.
...
- [NES Architecture](/vault/systems/nintendo-entertainment-system)
