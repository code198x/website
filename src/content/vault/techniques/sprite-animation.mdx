...
sources:
  c64_pointer_swap:
    - title: "Commodore 64 Programmer's Reference Guide, Commodore Business Machines"
      kind: book
      page: "132-133"
      note: "Sprite pointers: \"Even though each sprite takes only 63 bytes to define, one more byte is needed as a place holder at the end of each sprite. Each sprite, then, takes up 64 bytes.\" — \"These 8 bytes are always located as the last 8 bytes of the 1K chunk of screen memory. Normally, on the Commodore 64, this means they begin at location 2040 ($07F8 in HEX). However, if you move the screen, the location of your sprite pointers will also move.\" — \"LOCATION = (BANK * 16384) + (SPRITE POINTER VALUE * 64)\""
    - title: "Commodore 64 Programmer's Reference Guide, Commodore Business Machines"
      kind: book
      page: "166-172"
      note: "The Dancing Mouse — A Sprite Program Example: \"a fun sprite program called 'Michael's Dancing Mouse.' This program uses three different sprites in a cute animation with sound effects\" — line 20: \"We are going to use ONE SPRITE (sprite 0) in this animation, but we are going to use THREE sets of sprite data to define three separate shapes. To get our animation, we will switch the POINTERS for sprite 0 to the three places in memory where we have stored the data which defines our three different shapes. The same sprite will be redefined rapidly over and over again as 3 different shapes to produce the dancing mouse animation.\" — \"One sprite can have many different shapes, simply by changing the POINTER SETTING FOR THAT SPRITE\" — line 45: \"Changing this pointer to the locations of the other two sprite shapes is the secret of using one sprite to create an animation that is actually three different shapes.\" — line 80: \"POKE2040,P Sets the SPRITE POINTER\"; \"FORT=1TO60:NEXT A simple time delay loop which sets the speed at which the mouse dances.\""
  c64_reviewers:
    - ref: magazines/zzap-64
      ...
      date: "1985-05"
      page: "80-81"
      note: "Tir Na Nog, Gargoyle Games, £9.95 cass, keys only. Strapline \"Stunningly animated arcade adventure\" — \"You control a superbly animated Celtic warrior, Cuchulainn\" — \"The first thing that strikes you about this game is the size and superb characterisation of Cuchulainn. Twice as large as most computer heroes he strolls into his quest with long, flowing strides, his hair flapping in the wind.\" — \"you will meet horrible creatures - these too are excellently animated. They walk along the paths and career into you\""
---
...
Commodore taught this in the Programmer's Reference Guide. Each sprite "takes up 64 bytes", the eight pointers are "the last 8 bytes of the 1K chunk of screen memory" — 2040 ($07F8) unless the screen has moved — and a pointer value times 64 addresses anywhere in the VIC-II's 16K bank. The guide's worked example, "Michael's Dancing Mouse", uses "ONE SPRITE (sprite 0)" and "THREE sets of sprite data to define three separate shapes", and animates by switching the pointer between them: "The same sprite will be redefined rapidly over and over again as 3 different shapes to produce the dancing mouse animation." Its animation rate is a BASIC delay loop, `FORT=1TO60:NEXT`, "which sets the speed at which the mouse dances".

Reviewers noticed when it was done well. ZZAP! 64's first issue gave Gargoyle Games' *Tir Na Nog* the strapline "Stunningly animated arcade adventure": Cuchulainn is "twice as large as most computer heroes" and "strolls into his quest with long, flowing strides, his hair flapping in the wind", and the creatures "too are excellently animated".
