---
ai_generated: true
reviewed: false
title: "Bemani"
subtitle: "Konami's music division"
summary: "Bemani developed Konami's rhythm game empire including Beatmania, Dance Dance Revolution, and Guitar Freaks, dominating Japanese arcades through dedicated music gaming."
category: "companies"
platforms: ['arcade', 'sony-playstation', 'sony-playstation-2']
tags: ['developer', 'konami', 'rhythm']
founded: 1997

---

Rhythm gaming as a business division. Bemani, a name Konami took from its game [*Beatmania*](/vault/games/beatmania), became Konami's dedicated music game brand, producing an interconnected family of arcade experiences. Each game featured different controllers—turntables, dance pads, guitars, drums, buttons—but shared song libraries and visual styles. Japanese arcades filled with Bemani cabinets, creating a distinct gaming subculture.

## Fast facts

- **Founded:** 1997.
- **Parent:** [Konami](/vault/companies/konami).
- **Name origin:** shortened from *Beatmania* (*Edge* issues 70, 124 and 211).
- **Focus:** Arcade rhythm games.

## Core franchises

| Game | Year | Controller |
|------|------|------------|
| Beatmania | 1997 | Keys + turntable |
| DDR | 1998 | Dance pad |
| Pop'n Music | 1998 | 9 buttons |
| Guitar Freaks | 1999 | Guitar |
| Drum Mania | 1999 | Drum kit |
| Beatmania IIDX | 1999 | 7 keys |

## Interconnected ecosystem

| Feature | Implementation |
|---------|----------------|
| Shared songs | Cross-game music |
| Link play | Multi-cabinet connection |
| E-amusement | Network features |
| Event songs | Limited releases |

## Arcade dominance

| Region | Status |
|--------|--------|
| Japan | Dominant presence |
| Asia | Strong following |
| West | Niche but devoted |
| Home ports | Mixed availability |

## Cultural impact

| Influence | Note |
|-----------|------|
| Rhythm genre | Defined format |
| Controller design | Industry template |
| Music licensing | Game soundtracks |
| Tournament scene | Competitive play |

## See also

- [Beatmania](/vault/games/beatmania)
- [Dance Dance Revolution](/vault/games/dance-dance-revolution)
- [Konami](/vault/companies/konami)
