...
A [held tone](/patterns/nintendo-nes/assembly/audio/square-wave) is a note; a sound effect *moves*. The APU has hardware to move it for you: an **envelope** that fades a channel's volume from full to nothing, a **sweep** unit that slides a pulse channel's pitch up or down, and a **noise** channel of pseudo-random hiss for anything that isn't a pitch — footsteps, explosions, drums. Set them up, write the trigger register once, and the effect plays and stops itself. No per-frame code, no interrupt: the classic NES "pew" and "boom" are each a few register writes and then silence.
...
**Patterns:** [A Square Wave](/patterns/nintendo-nes/assembly/audio/square-wave)
