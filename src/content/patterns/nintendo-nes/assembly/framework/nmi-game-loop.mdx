...
| Aspect | Cost |
|--------|------|
| CPU | 513–514 cycles for DMA + overhead |
...
1. The PPU generates NMI at the start of VBlank — about 60 NMIs per second on NTSC; VBlank lasts ~2273 CPU cycles per frame.
2. Your NMI handler runs automatically.
3. DMA the OAM buffer to the PPU (513–514 CPU cycles, fully automatic — odd starting cycles add one).
4. Update any other PPU state (palettes, nametables) if needed.
5. Reset scroll position (important after any `$2006`/`$2007` access — those writes corrupt the internal scroll register).
6. Main loop continues with game logic.

## One-frame latency

This pattern carries a deliberate one-frame display latency. The NMI runs in roughly this order:

1. DMA the *previous frame's* `oam_buffer` contents into PPU OAM (this is what the player sees this frame).
2. Update `oam_buffer` from current `player_x`/`player_y` (will be visible next frame).

So game state updated in the main loop becomes visible one frame later. This is normal NES practice and matches how most published games work — it's how you get a stable, glitch-free display. If you ever see "input feels delayed by one frame," this is why.
...
**Patterns:** [Controller Reading](/patterns/nintendo-nes/assembly/input/controller-reading), [Palette Loading](/patterns/nintendo-nes/assembly/rendering/palette-loading)

**Vault:** [NES](/vault/systems/nintendo-entertainment-system)
