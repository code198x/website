...
One channel plays [one note](/patterns/nintendo-nes/audio/square-wave); a [sound effect](/patterns/nintendo-nes/audio/sound-effects) makes it move. Music is the step up: **several channels at once**, each handed a fresh note on a regular beat. On the NES that beat is already ticking — the [NMI fires every frame](/patterns/nintendo-nes/framework/nmi-game-loop), 60 times a second on NTSC. Count those frames into rows, and on each row look up a **timer value per channel** from a note table and write it. Pulse 1 takes the lead, pulse 2 the harmony, the triangle the bass; the noise channel drops in drums. That loop — a tick, a pattern, a note table — is every NES sound driver in miniature.
...
**When to avoid:** A single reactive bleep; that's a [sound effect](/patterns/nintendo-nes/audio/sound-effects), not a sequencer.
...
- **Drums** — the [noise channel](/patterns/nintendo-nes/audio/sound-effects) triggered from its own pattern row; it takes a period index, not a note.
...
**Patterns:** [Sound Effects](/patterns/nintendo-nes/audio/sound-effects), [A Square Wave](/patterns/nintendo-nes/audio/square-wave), [NMI Game Loop](/patterns/nintendo-nes/framework/nmi-game-loop)
