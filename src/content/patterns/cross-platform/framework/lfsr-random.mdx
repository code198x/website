...
taught_in: "Maths for Games, Unit 6 (The Craft)"
...
**Always seed with 1 or higher.** If your seed value can come from external state (RAM contents, player input timing), test for zero and substitute 1:
...
The same caveat applies to corruption: if any code path can write to `random_seed` (e.g. accidental zero-page clear), guard the LFSR routine with the same check.
