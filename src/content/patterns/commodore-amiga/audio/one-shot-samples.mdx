...
Don't silence the channel in the handler — hand it the **next** sample instead (new `LC`/`LEN`/`PER`/`VOL`) — and you've built the core of a sample sequencer. A MOD player is exactly this on all four channels: a per-channel interrupt, a pattern of notes, and the [period table](/patterns/commodore-amiga/audio/playing-a-sample) turning note numbers into pitches.

## Related

**Patterns:** [Playing a Sample](/patterns/commodore-amiga/audio/playing-a-sample)
