...
start:
            ...
            move.w  #428,AUD0PER(a5)     ; period 428 = the note C-2 (~8.3 kHz PAL)
...
Trackers pick periods from a fixed table where each entry is a semitone; period **428 is C-2**, halve it (214) and you're an octave up at C-3. To play a recorded sample at the right pitch you set the period to match how it was sampled.
...
Enable a channel and Paula plays `AUD0LC`/`AUD0LEN` and then **loops it forever** — perfect for a sustained instrument. To play a sound *once*, you use Paula's channel interrupt, which fires as the channel loads the sample at the start of each pass: stop the channel on the second one, or queue a silent word on the first; see [One-Shot Samples](/patterns/commodore-amiga/assembly/audio/one-shot-samples).

## Related

**Patterns:** [VBlank Game Loop](/patterns/commodore-amiga/assembly/framework/vblank-game-loop)
