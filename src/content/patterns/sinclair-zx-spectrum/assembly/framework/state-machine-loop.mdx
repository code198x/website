...
**Note on `key_pressed` values:** this pattern uses an extended keyboard reader that returns:

| Value | Key | Used for |
|------:|-----|----------|
| 0 | (none) | No input |
| 1-4 | Up/Down/Left/Right | Movement (per [keyboard-reading](/patterns/sinclair-zx-spectrum/assembly/input/keyboard-reading)) |
| 5 | Space | State transitions (start, restart, pause toggle) |

The basic `keyboard-reading` pattern only returns 0-4. Extend it to also check port `$7FFE` bit 0 (Space) and return 5 when pressed.
...
**Patterns:** [Game Loop (HALT)](/patterns/sinclair-zx-spectrum/assembly/framework/game-loop-halt), [Keyboard Reading](/patterns/sinclair-zx-spectrum/assembly/input/keyboard-reading)
