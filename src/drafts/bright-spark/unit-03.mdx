...
Continue with the complete program from [Give a choice a signal](../unit-02/),
...
Keep every other line. Enter `LET` with L after the line number and `PRINT`
with P. On the Host Keyboard used in the opening lessons, left Option/Alt+4
enters `$`, left Option/Alt+P enters a quote, left Option/Alt+L enters `=`,
and left Option/Alt+O enters `;`. `AT` is left Option/Alt+I, and `STOP` is
left Option/Alt+A. Release each chord before the next character.
...
`LEN s$` gives the number of characters in `s$`. Enter `LEN` by pressing
Shift+left Option/Alt, releasing both keys, then pressing K. It is a function:
it works out a value that `PRINT` can use. It does not change the string.

`s$(1)` selects the character at position 1. Enter the brackets with left
Option/Alt+8 and left Option/Alt+9. Sinclair BASIC counts string positions
...
<details>
<summary>The first character and the length both print as 3. Are they the same kind of value?</summary>
...
Enter `+` with left Option/Alt+K. For strings, `+` joins the characters together;
...
<details>
<summary>What changes after appending the choice?</summary>
