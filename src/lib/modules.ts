 ...
 */
export async function getModules(platform: string, track: 'assembly' | 'basic' | 'amos'): Promise<Module[]> {
 ...
 */
export async function getModulesEntry(platform: string, track: 'assembly' | 'basic' | 'amos'): Promise<ModulesEntry | undefined> {
  const allGames = await getCollection("modules");
