  ...
  return (
    <div className="register-display bg-slate-900 p-4 rounded-lg font-mono">
      ...
      <div className="status-flags">
        ...
        <div className="flex gap-2 mb-2">
          {["N", "V", "-", "B", "D", "I", "Z", "C"].map((flag, i) => {
            if (flag === "-") {
              return (
                <div key={i} className="flag-box inactive">
                  1
                </div>
              );
            }
            ...
            const changed =
              previousRegisters &&
              registers[flag as keyof RegisterState] !==
                previousRegisters[flag as keyof RegisterState];
      ...
      <div className="mt-4 p-2 bg-slate-800 rounded">
        <div className="text-xs text-slate-400">Last Instruction Effect:</div>
        {previousRegisters && (
...
function describeChanges(current: RegisterState, previous: RegisterState): string {
  ...
  if (current.A !== previous.A) {
    changes.push(
      `A: $${previous.A.toString(16).toUpperCase()} → $${current.A.toString(16).toUpperCase()}`
    );
  }
  if (current.X !== previous.X) {
    changes.push(
      `X: $${previous.X.toString(16).toUpperCase()} → $${current.X.toString(16).toUpperCase()}`
    );
  }
  if (current.Y !== previous.Y) {
    changes.push(
      `Y: $${previous.Y.toString(16).toUpperCase()} → $${current.Y.toString(16).toUpperCase()}`
    );
  }
