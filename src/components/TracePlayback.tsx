    ...
    <div className="trace-playback bg-slate-900 p-4 rounded-lg">
          ...
          <div className="bg-slate-800 rounded p-2 h-64 overflow-y-auto">
            {trace.length === 0 ? (
              ...
              trace.map((entry, i) => (
                <div
                  key={i}
                  className={`mb-2 p-2 rounded text-xs ${i === currentStep ? "bg-slate-700" : ""}`}
                >
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs font-mono">
          <div className="text-slate-300">
            ...
            <span className="text-white">
              ${currentState.PC.toString(16).toUpperCase().padStart(4, "0")}
            </span>
          </div>
          <div className="text-slate-300">
            A:{" "}
            <span className="text-white">
              ${currentState.A.toString(16).toUpperCase().padStart(2, "0")}
            </span>
          </div>
          <div className="text-slate-300">
            X:{" "}
            <span className="text-white">
              ${currentState.X.toString(16).toUpperCase().padStart(2, "0")}
            </span>
          </div>
          <div className="text-slate-300">
            Y:{" "}
            <span className="text-white">
              ${currentState.Y.toString(16).toUpperCase().padStart(2, "0")}
            </span>
          </div>
          <div className="text-slate-300">
            SP:{" "}
            <span className="text-white">
              ${currentState.SP.toString(16).toUpperCase().padStart(2, "0")}
            </span>
          </div>
          <div className="text-slate-300">
          ...
          <div className="text-slate-300">
            Status:{" "}
            <span className={currentState.halted ? "text-red-400" : "text-green-400"}>
      ...
      <div className="mt-4 p-3 bg-blue-900 rounded">
        <div className="text-xs text-blue-200">
          <strong>Learning Note:</strong> This is a simplified educational CPU. It demonstrates core
          concepts without requiring system ROMs or copyrighted code. Real 6502 processors have more
          instructions and complex timing, but the principles are the same!
        </div>
      </div>
    </div>
  );
