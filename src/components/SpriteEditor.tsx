...
export default function SpriteEditor({ 
  ...
  const initGrid = () => Array(height).fill(0).map(() => Array(width).fill(0));
  ...
  const colors = {
    ...
    nes: [
      '#000000', '#FCFCFC', '#F8B800', '#F87858',
      '#AC7C00', '#00A800', '#00B800', '#503000'
    ],
    atari800: [
      '#000000', '#FFFFFF', '#FFB810', '#FF6821',
      '#FF3121', '#CD1F39', '#942B52', '#5A316B'
    ],
    spectrum: [
  ...
  // Fill sprite
  const fillSprite = () => {
    const filled = Array(height).fill(0).map(() => Array(width).fill(currentColor));
    setPixels(filled);
  ...
  // Invert sprite
  const invertSprite = () => {
    const inverted = pixels.map(row => 
      row.map(pixel => pixel === 0 ? currentColor : 0)
    );
    setPixels(inverted);
  ...
  const getExportData = () => {
    ...
    const formats = {
      ...
      assembly: '.byte ' + data.map(b => '$' + b.toString(16).toUpperCase().padStart(2, '0')).join(', ')
    };
