---
layout: ../../layouts/VaultLayout.astro
title: "Zzap!64"
subtitle: "Britain’s loudest Commodore magazine"
summary: "Launched in 1985, *Zzap!64* mixed enthusiastic reviews, dev diaries, and irreverent humour—shaping how a generation discovered C64 games."
system_tags: [C64]
tech_tags: ["Magazines", "Games media"]
years: [1985, 1994]
---

> [📷 suggested: collage of Zzap!64 covers and reviewer caricatures]

## Overview
Created by Newsfield Publications, *Zzap!64* delivered monthly deep dives into Commodore 64 software. Its reviewers (“The Zzap! Rrap”) wrote in first-person excitement, backed by screenshots, tips, and candid developer interviews.

## Fast facts
- **Editors:** Chris Anderson, Roger Kean, and Franco Frey shaped the early tone.  
- **Features:** “Sizzler” awards for outstanding games, developer diaries (Andrew Braybrook’s *Paradroid* journal), and the cheeky letters page.  
- **Spin-offs:** *Zzap!64 Amiga* evolved into *Amiga Format*; modern fan revivals continue online.

## Lesson connections
- BASIC lessons reference *Zzap!64* diaries to show how developers communicated progress.  
- Vault cross-links encourage learners to read historical reviews when studying games like *Uridium* or *Paradroid*.  
- Editorial voice informs our project’s tone—enthusiastic, inviting, and technically honest.

## Cultural impact
*Zzap!64* championed bedroom coders, giving them celebrity status and providing the first real-time postmortems in print. Its mix of art, humour, and detail remains a template for retro-inspired media.

## See also
- [Commodore 64](/vault/commodore-64)  
- [Andrew Braybrook](/vault/andrew-braybrook)  
- [Paradroid](/vault/paradroid)  
- [Uridium](/vault/uridium)  
- [The Rise of the Bedroom Coder](/vault/bedroom-coder)
