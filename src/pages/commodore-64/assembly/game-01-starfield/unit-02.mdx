...
Load `starfield.prg` in VICE. Push the joystick (or use the keyboard arrows if VICE maps them) — your ship moves in all four directions.

## What's Changed

Unit 1's program ran its setup code and then sat in an infinite loop doing nothing. This version replaces that loop with a **game loop** — code that runs once per frame, every frame, forever. Each pass reads the joystick and moves the ship.
